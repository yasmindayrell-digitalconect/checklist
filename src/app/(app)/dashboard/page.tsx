...
export default async function Page() {
  ...
  // ------------------------------------
  // Shared seller filter (admin whitelist)
  // ------------------------------------
  const sellerIds =
    session.role === "admin"
  ...
  // -----------------------------
  // ✅ aqui você mantém sua query de open budgets
  // só garanta que você tenha:
  ...
  const { rows: openBudgetRows } = await radarPool.query<Row>(
    sqlOpenBudgets,
    openBudgetArgs
  );
...
`;
 ...
 const sellerKpis: SellerKpiRow[] = (kpiRowsRaw as any[]).map((r) => ({
  ...
  return (
    <DashboardClient
      openBudgetClients={openBudgetClients}
