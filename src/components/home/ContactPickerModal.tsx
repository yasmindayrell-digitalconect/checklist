...
type Props = {
  ...
  clientName: string;
  contacts: ContatoRow[];
  onPick: (c: ContatoRow) => void;
};

export default function ContactPickerModal({ open, onClose, clientName, contacts, onPick }: Props) {
  if (!open) return null;

  const valid = contacts.filter((c) => !!c.telefone);
